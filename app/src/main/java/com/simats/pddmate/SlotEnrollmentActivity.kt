...
import com.simats.pddmate.databinding.ActivitySlotEnrollmentBinding
import okhttp3.*
...
class SlotEnrollmentActivity : AppCompatActivity() {
    ...
    private fun loadSlotsForEnrollment() {
        ...
        val url = "http://14.139.187.229:8081/pddmate/get_enrollments.php?student_user_id=$currentStudentUserId"
    ...
    private fun requestEnrollment(slot: EnrollmentSlot) {
        ...
        val request = Request.Builder()
            .url("http://14.139.187.229:8081/pddmate/enroll_project.php")
            .post(requestBody)
            .build()
        client.newCall(request).enqueue(object : Callback {
            override fun onFailure(call: Call, e: IOException) {
